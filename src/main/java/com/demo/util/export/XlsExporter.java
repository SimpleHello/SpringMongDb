...
public class XlsExporter {
    ...
    public static interface DataRetriver extends Retriver{
        List retrive(int start, int limit);
    }
    public static void export(OutputStream out, Retriver dataRetriver, String[] properiesName, String[] headsName, Class beanClass, String exportFileName, boolean isZipped) throws IOException {
        export(out, dataRetriver, properiesName, headsName, beanClass, exportFileName);
    }

    public static void export(OutputStream out, Retriver dataRetriver, String[] properiesName, String[] headsName, Class beanClass, String exportFileName) throws IOException {
        int count = 1;
		...
		Row row = null;
		Cell cell = null;
//		cell.setCellValue(new HSSFRichTextString(exportFileName));
...
//		cell.setCellStyle(getTitleStyle(wb));
        ...
        List data;
        do {
            data = dataRetriver.retrive(start, limit);
            for(int i=0;i<data.size();i++){
    			...
    			for (int iCol = 0; iCol < properiesName.length; iCol++) {
    				try {
    					...
    					// Field field = clazz.getDeclaredField(key[iCol]);
